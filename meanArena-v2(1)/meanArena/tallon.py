    ...
    def makeMove(self):
        ...
        try:
            ...
            if int(vi2.policy[current_tallon]) == self.move[0]:
                ...
                return Directions.SOUTH# when the optimum policy is zero 
...
#when the grid is partialy observable the below code will run to find the bonus
    ...
    # the below code is run to get the probability and reward array required to do the value iteration 
    def probability(self):
       # the below code will give the length and breadth of the grid
        ...
        action_N_S_E_W=(.025, .025, config.directionProbability, 0.)  #West,East,North,South     
          #total number of states are the total number of grids present in the world
        num_states = grid_size[0] * grid_size[1]
        #total number of actons are the directions in which tallon is allowed to move
        num_actions = 4
        # these below two are the arrays to store the probability and reward
        Pr = np.zeros((num_actions, num_states, num_states))
        Re = np.zeros((num_states, num_actions))
        try:
            #this gives the position of the tallon
            myPosition = self.gameWorld.getTallonLocation()
            tallon_position = lambda x: np.ravel_multi_index(x, (10,10))
            ...
            #these below three arrays are used to store all the locations of the bonus,pits and meanies
            bonuses_loc=[]
            pits_loc=[]
            meanies_loc=[] 
           #this code  creates an array of the coordinates and used the min function to get the location closest to Tallon so it could go to that first
            ...
            # this below function is used to avoid getting errors form hitting the walls
